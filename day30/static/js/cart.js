...
let book_title = document.querySelector('.books-title');
...
function changeCount(){
    if (addCart.innerText == "Səbətə əlavə et"){
        ...
        newItem.innerHTML = `<p><i class="fa-solid fa-xmark text-danger" id="deleteBook"></i> ${book_title.innerHTML}</p><p><span id="bookPrice">${price.innerHTML}</span>AZN</p>`
        ...
        sumBook.innerText = ParseFloat(parseFloat(sumBook.innerText) + parseFloat(price.innerHTML));
    }
    else{
        removeBook();
    }
...
function addBooks(){
    for (let i = 0; i < 2; i ++){
        ...
        sumBook.innerText = ParseFloat(priceBook);
    }        
}
...
function deleteBookFunction(bookItem){
    ...
    bookItem.addEventListener('click', ()=>{
        if (bookItem.parentElement.innerHTML == `<i class="fa-solid fa-xmark text-danger" id="deleteBook" style="cursor: pointer;"></i> ${book_title.innerHTML}`){
            removeBook();
        }
        ...
        deletedPrice = parseFloat(deletedPrice.substr(0,2));
        console.log(deletedPrice, '   --   ', sumBook.innerText)
        sumBook.innerText = ParseFloat(sumBook.innerText - deletedPrice);
        listCount.innerText = dropdownMenu.childElementCount;
        if (listCount.innerHTML == 0){
